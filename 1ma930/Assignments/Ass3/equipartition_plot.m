function equipartition_plot(n)
    ...
    curve = plot(x, y, 'k-');
    hold on;
    pts = plot(x_points, y_points, 'ro', 'MarkerSize', 8, 'MarkerFaceColor','r');
    hold off;
    title(sprintf('Equipartition with n = %d', n));
    axis equal;
    grid on;
    legend([curve, pts], {'Curve', 'Bisection Points'}, 'Location', 'best');
end